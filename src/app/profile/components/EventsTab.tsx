...
export function EventsTab({
  ...
  const attendedIds = new Set(
    (user.eventsAttended || []).map((e: any) => (e._id || e.id || e).toString())
  );

  // Compute user's registered/participated events
  const myRegisteredEvents = myEvents && myEvents.length >= 0
    ? myEvents
    : allEvents.filter((ev) => {
        const evId = (ev._id || ev.id)?.toString();
        if (evId && attendedIds.has(evId)) return true;
        if (Array.isArray(ev.attendees) && ev.attendees.some((a: any) => (a._id || a.id || a)?.toString() === userId)) return true;
        if (Array.isArray(ev.approvedParticipants) && ev.approvedParticipants.some((p: any) => (p.userId?._id || p.userId || p._id)?.toString() === userId || p.studentId === user.studentId)) return true;
        if (Array.isArray(ev.pendingParticipants) && ev.pendingParticipants.some((p: any) => (p.userId?._id || p.userId || p._id)?.toString() === userId || p.leaderStudentId === user.studentId)) return true;
        return false;
      });
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="bg-surface-elevated border-[1.5px] border-border-brutalist dark:border-border-default rounded-xl shadow-[4px_4px_0px_0px_var(--border-brutalist)] dark:shadow-[4px_4px_0px_0px_var(--border-default)] p-4 sm:p-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5 pb-3 border-b-[1.5px] border-border-default">
          <div>
            <h2 className="font-heading text-lg sm:text-xl font-extrabold text-text-primary m-0">My Events ({myRegisteredEvents.length})</h2>
          ...
          >
            {showBrowseAll ? "Hide Club Events Schedule" : "Browse Upcoming Club Events"}
          </Button>
        </div>
        ...
        {myRegisteredEvents.length > 0 ? (
          ...
          <div className="text-center py-10 text-text-secondary">
            <Calendar size={40} className="mx-auto mb-2 opacity-40 text-accent-primary" />
            <h3 className="text-base font-bold text-text-primary mb-1">No Events Registered Yet</h3>
            <p className="text-xs sm:text-sm text-text-secondary max-w-md mx-auto mb-4">
              You haven&apos;t registered for any club events or contests yet. Browse upcoming club events to participate!
            </p>
            {!showBrowseAll && (
              <Button size="sm" onClick={() => setShowBrowseAll(true)}>
      ...
      {/* Available Upcoming Events (Expandable) */}
      {showBrowseAll && (
        <div className="bg-surface-elevated border-[1.5px] border-border-brutalist dark:border-border-default rounded-xl shadow-[4px_4px_0px_0px_var(--border-brutalist)] dark:shadow-[4px_4px_0px_0px_var(--border-default)] p-4 sm:p-6 animate-[fadeIn_0.25s_ease-out]">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5 pb-3 border-b-[1.5px] border-border-default">
            <div>
            ...
            <Button size="sm" variant="ghost" onClick={() => setShowBrowseAll(false)}>
              Hide
            </Button>
          </div>
